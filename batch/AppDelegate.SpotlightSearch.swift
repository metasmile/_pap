...
class SpotlightSearchAppDelegate: NSObject, UIApplicationDelegate{
    ...
    public static func launchAppIfNeededWithSearchable(activity:NSUserActivity?=SpotlightSearchAppDelegate.selectedUserActivity) -> Bool{
        ...
        AppCenter.default.openApp(identifier: app.info.identifier, options: AppLaunchOptions(options: [.NSUserActivity: activity])) { _ in
            AppCenter.default.currentInstanceAs(UIApplicationDelegateLaunchableApp.self)?.didLaunchHandling(with:activity)
        }
    ...
    func indexDefaultSearchableItems(){
        ...
        //main item
        let csItemAttr = CSSearchableItemAttributeSet(itemContentType: UTI.image.rawValue)
        csItemAttr.title = batchStrings.name
        csItemAttr.contentDescription = batchStrings.title
