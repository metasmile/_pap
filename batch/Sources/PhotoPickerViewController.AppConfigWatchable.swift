...
extension PhotoPickerViewController:AppDockViewControllerAppConfigWatchableDelegate{
    ...
    func registerWatchingAppConfig() {
        AppCenter.default.watch(\.currentIdentifier, options: [.new, .old, .initial]) { (appCenter, dict) in
            ...
            AppCenter.default.currentInstanceAs(AdjustmentsApp.self)?.config?.watch(\.filter, id: "picker\(AdjustmentsApp.info.identifier)") { (config, changed) in
                if let value = config.filter, !AppCenter.default.task.isRunning {
                    self.appendImageEditState(value)
                }
            }

            AppCenter.default.currentInstanceAs(DepthEditorApp.self)?.config?.watch(\.filter, id: "picker\(DepthEditorApp.info.identifier)") { (config, changed) in
                if let value = config.filter, !AppCenter.default.task.isRunning {
                    self.appendImageEditState(value)
                }
            }
            
            AppCenter.default.currentInstanceAs(RawEditorApp.self)?.config?.watch(\.filter, id: "picker\(RawEditorApp.info.identifier)") { (config, changed) in
                if let value = config.filter, !AppCenter.default.task.isRunning {
                    self.appendImageEditState(value)
                }
            }
            
            AppCenter.default.currentInstanceAs(CurveEditorApp.self)?.config?.watch(\.filter, id: "picker\(CurveEditorApp.info.identifier)") { (config, changed) in
                if let value = config.filter, !AppCenter.default.task.isRunning {
                    self.appendImageEditState(value)
                }
            }
    ...
    func unregisterWatchingAppConfig() {
        ...
        AppCenter.default.currentInstanceAs(CurveEditorApp.self)?.config?.unwatch(\.filter, forIds:["picker\(CurveEditorApp.info.identifier)"])
