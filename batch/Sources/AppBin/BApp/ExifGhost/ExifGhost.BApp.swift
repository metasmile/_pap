...
public class ExifGhostApp: NSObject, PropertyWatchable,BApp,
    ...
    public static let info = AppInfo(
            ...
            , icon: AppIcon(source: R.image.exifGhostBAppIcon.name, style: .themeColor)
            ...
            if let item = item, let url = item.input.fullSizeImageURL{
                ...
                , let metadata = data.getMetadata(){
                    ...
                    do{
                        ...
                        result = PHAssetResultItem(asset:param, editingResultItems: [PHAssetEditingResultItem(url: item.output.renderedContentURL, resourceType: .photo)], contentEditingOutput:item.output)
                    }catch _ {}
                }
