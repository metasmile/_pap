//
// Created by BLACKGENE on 29/06/2018.
...
public struct ContactsUtil {
    ...
    public func contactsToVCardConverter(contacts: [CNContact]) ->  ContactsToVCardResult {

        var vcardFromContacts: Data = Data()
        do {
            try vcardFromContacts = CNContactVCardSerialization.data(with: contacts)
            return ContactsToVCardResult.Success(response: vcardFromContacts)
        } catch {
            return ContactsToVCardResult.Error(error: error)
        }
    ...
    public func VCardToContactConverter(data: Data) ->  VCardToContactResult {
        var contacts: [CNContact] = [CNContact]()
        do {
            try contacts = CNContactVCardSerialization.contacts(with: data) as [CNContact]
            return VCardToContactResult.Success(response: contacts)
        } catch {
            return VCardToContactResult.Error(error: error)
        }
    }
